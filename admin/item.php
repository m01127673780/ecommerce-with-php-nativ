  ...
  <table class="table table-striped main-table table table-bordered text-center">
    <thead>
      <tr>
        ...
        <th >Description</th>
        <th >Price</th>
        <th style="" >Adding Date</th> 
 
        <th >Category</th>
        <th >Username</th>
        <th > <span class="hiddin-in-768" > Control</span></th>  
       </tr>
    </thead>
   <tbody>
                <?php
        ...
        /*-----------------------------------*/
          ...
          echo"</td>";
               ...
               echo "<td class='control text-center'> 
                          <a href='?action=edit&id=" .$row['id']. "' class='btn btn-success btn-smol'><i class='fas fa-edit'></i>
    ...
    <form action="?action=insert" method="post" enctype="multipart/form-data">
        <div class="form-group row">
            ...
            <div class="col-sm-10 col-md-6">
                <input type="text" name="name" class="form-control" id="name" placeholder="Name Of The Item" minlength="3" required />
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-2 col-form-label form">Description</label>
            <div class="col-sm-10 col-md-6">
                <input type="text" name="description" class="form-control" id="name" placeholder="Description Of The Item" minlength="12" required />
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-2 col-form-label form">Price</label>
            <div class="col-sm-10 col-md-6">
                <input type="number" name="price" class="form-control" id="name" placeholder="Price Of The Item" required />
            </div>
        </div>
        <div class="form-group row">
            <label class="col-sm-2 col-form-label form">Country</label>
            <div class="col-sm-10 col-md-6">
                <input type="text" name="country" class="form-control" placeholder="Country Of The Item" minlength="3" required />
            </div>
        </div>
        <div class="form-group row">
        ...
        <div class="form-group row">
            <label class="col-sm-2 col-form-label form">Product Images</label>
            <div class="col-sm-10 col-md-6">
    ...
    elseif($action == 'insert'){
         ...
         if(!empty($err_form)){
                  ...
                  $count = $stmt->rowCount();
                     ...
                     /* Start Multiple Upload Files */
                 if(!empty($_FILES['imgs']['name'])){
                 $imgs = $_FILES['imgs'];
                $img_names      = $imgs['name'];
                $img_types      = $imgs['type'];
                $img_sizes      = $imgs['size'];
                $img_tmps       = $imgs['tmp_name'];
                    
                $files_count = count($img_names);
                for($i = 0; $i < $files_count;$i++)
                {
                    if($img_sizes[$i] > 4194304){
                       $err_form[] = 'Image Number '.($i+1).' Size Can\'t be more than 4 M.B'; 
                    }
                    
                $img_extensions[$i] = explode('.',$img_names[$i]);
                $img_extensions[$i] = strtolower(end($img_extensions[$i]));
                    
                  if(!empty($img_names[$i]) && !in_array($img_extensions[$i],$img_allow_extension)){
                    $err_form[] = 'File Number '.($i+1).' Extension Isn\'t Allowed ';
                  }
     
                if(empty($err_form))
                {
                    $names_imges ='';
                        $img_names[$i] = rand(0,1000000)."_".$img_names[$i];
                       $paths = '../uploads/product-img/'.$img_names[$i];
                        move_uploaded_file($img_tmps[$i], $paths);
                    if($files_count >= 1 && $files_count > $i){
                        $sebartor = ',';
                    }else{
                        $sebartor = '';
                    }
                    foreach($img_names as $imgs_names)
                    {
                        $names_imges .= $imgs_names.$sebartor;
                    }
                         ...
                         if(empty($err_form))
                {
                           $stmt = $con->prepare("update items set images = ? where name =? AND description =? AND price =? AND country_made =? AND status =? AND
                  member_id =? AND cat_id =?");
                        $stmt->execute(array($names_imges, $name, $description, $price, $country, $status,$member_id,$cat_id));
                        $count = $stmt->rowCount();
            }
            }  
    ...
    <div class="container">
        <form action="?action=update" enctype="multipart/form-data" method="post">
            ...
            <div class="form-group row">
                <label class="col-sm-2 col-form-label form">Name</label>
                <div class="col-sm-10 col-md-6">
                    <input type="text" name="name" class="form-control" id="name" placeholder="Name Of The Item" value="<?php echo $row['name'];?>" minlength="3" required />
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label form">Description</label>
                <div class="col-sm-10 col-md-6">
                    <input type="text" name="description" class="form-control" placeholder="Description Of The Item" value="<?php echo $row['description'];?>" minlength="12" required />
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label form">Price</label>
                <div class="col-sm-10 col-md-6">
                    <input type="number" name="price" class="form-control" placeholder="Price Of The Item" value="<?php echo $row['price'];?>" required />
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label form">Country</label>
                <div class="col-sm-10 col-md-6">
                    <input type="text" name="country" class="form-control" placeholder="Country Of The Item" value="<?php echo $row['country_made'];?>" minlength="3" required />
                </div>
            </div>
            <div class="form-group row">
                <label class="col-sm-2 col-form-label form">Product Image</label>
                ...
                if(empty($err_form))
                {
                    ...
                    foreach($img_names as $imgs_names)
                    {
                        $names_imges .= $imgs_names.$sebartor;
                         ...
                         if(empty($err_form))
                {
                           $stmt = $con->prepare("update items set images = ? where id = ?");
                        $stmt->execute(array($names_imges,$item_id));
                        $count = $stmt->rowCount();
            }
            }  
